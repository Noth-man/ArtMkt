...
func (m email_detail) body() string {
	return "To: " + m.to + "\r\n" +
		"Subject: " + m.sub + "\r\n\r\n" +
		m.msg + "\r\n"
}
...
func SendMail(c *gin.Context) {
	...
	url := config.Config.SendMailAddr + token
	msg := username + " 様 \n" + "仮登録が完了しました。\n" + "下記URLから本登録を完了してください。\n" + url
	sub := "【ArtMKT】本会員登録"
...
func SignupInputCheck(c *gin.Context) {
	karitoken := c.Param("karitoken")
	username, email, token := models.KariUserCheck(karitoken)
	c.HTML(200, "SignupInputCheck", gin.H{
		"email":     email,
		"username":  username,
		"token":     token,
...
func kari_registration(c *gin.Context) {
	...
	if models.UserCheck(username) == true && models.EmailCheck(e.Address) == true {
		...
		for {
			token = ""
			token = others.RandString(30)
			bool_token := models.KariTokenCheck(token)
			if bool_token == true {
				break
			}
		}

		returntoken := models.KariUserRegistration(username, e.Address, string(hash), token)
		redirectURL := "/signupinputcheck/" + returntoken
