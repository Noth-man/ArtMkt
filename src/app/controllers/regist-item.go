...
func CreateImage(file multipart.File, header *multipart.FileHeader, image_number, strproductid string) {
	...
	*/
	...
	key := "item/" + "productid" + strproductid + "/image" + image_number + ".jpg"
...
func ItemDelete(c *gin.Context) {
	...
	UserInfo.StripeAccount = sessiond.Get("StripeAccount")
	...
	if loginbool == true && models.CheckDeliveryStatusProductId(productid) == "なし" {
		...
		svc := s3.New(sess)
		_, err = svc.DeleteObject(&s3.DeleteObjectInput{Bucket: aws.String("artmkt"), Key: aws.String(key)})
