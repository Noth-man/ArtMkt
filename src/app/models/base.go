...
func ConnectionDB() {
	...
	//配送状況
	cmd4 := "CREATE TABLE IF NOT EXISTS delivery_status (id serial PRIMARY KEY, product_id INT, shipping INT, arrives INT, transfer INT);"
	...
	cmd7 := "CREATE TABLE IF NOT EXISTS kari_users (id serial PRIMARY KEY, username VARCHAR(50), password VARCHAR(255), email VARCHAR(255), token VARCHAR(255));"
	...
	cmd9 := `CREATE TABLE IF NOT EXISTS follow (
		id serial PRIMARY KEY, 
		user_id INT,
		follow_user_id INT
	);`
	_, err = DbConnection.Exec(cmd9)
	if err != nil {
		log.Fatalln(err)
	}
	...
	/*
		addcolumn := "ALTER TABLE kari_users ADD COLUMN token VARCHAR(255);"
